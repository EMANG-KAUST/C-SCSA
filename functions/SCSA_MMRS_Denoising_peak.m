f%% MRS signal Denosiing  using SCSA Method :
...
function [truth,harray,se,yscsa, h_op, Nh, Metabolite]=SCSA_MMRS_Denoising_peak(Denoising_coeff, ppm, yf, gm , fs , Th_peaks_ratio, width_peaks,lambda,yf0)
...
%     second_term=Cost_SNR;
    PartialSum=0;
PartialSum = 0;
for iii=1:Nh
    dot_pruduct_sum = 0;
        for jjj=1:size(psinnor,1)
            dot_pruduct_sum = y_res(jjj)*psinnor(jjj,iii)^2;      
        end
    PartialSum=PartialSum+dot_pruduct_sum^2;
end
Cost_peak=norm(y_res)^2;


second_term=PartialSum*lambda;
se(end+1)=second_term;
   % Cost_func_SNR(i)=Cost_SNR;
    Cost_func_MSE(i)=Cost_peak;
  % Cost_function(i)=Cost_SNR+Cost_peak;
  Cost_peak=norm(y_res);
   Cost_function(i)=second_term+Cost_peak;
    fprintf('.')
    harray(end+1)=second_term+Cost_peak;
    res=second_term+Cost_peak;
    truth(end+1)=res;
    hhh(end+1)=h;
end
