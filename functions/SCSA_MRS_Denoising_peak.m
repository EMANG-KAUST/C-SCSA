...
function [truth,harray,se,yscsa, h_op, Nh, Metabolite]=SCSA_MRS_Denoising_peak(Denoising_coeff, ppm, yf, gm , fs , Th_peaks_ratio, width_peaks,lambda,yf0)
...
%% Get the noise and metabolites locations
 [Noise,Metabolite]=find_metabolite_Noisy_Area(yf, Th_peaks_ratio, width_peaks)
   % Noise=[327 666];  Metabolite=[145,199]; %%%piece-wise regular testing, to be deleted later
 [m,n]=size(Metabolite);
 if m<1
     yscsa=1;
     return 
...
for i=1:length(hh)
    ...
    SNR_scsa=lambda*max(yscsa)/std(yscsa(Noise(1):Noise(2)));   %second term retaed to the  residual SNR
    se(end)=SNR_scsa;
